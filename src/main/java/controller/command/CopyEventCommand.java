...
public class CopyEventCommand implements ICommand {
  ...
  public CopyEventCommand(List<String> args, ICalendarModel model, String currentCalendar) {
    ...
    try {
      ...
      // 'on'
      if (index == args.size()) throw new IllegalArgumentException("Missing 'on' " +
            "keyword.");
      ...
      // sourceDateTime
      if (index == args.size()) throw new IllegalArgumentException("Missing source " +
            "datetime.");
      try {
      ...
      // '--target'
      if (index == args.size()) throw new IllegalArgumentException("Missing '--target' " +
            "keyword.");
      ...
      // 'to'
      if (index == args.size()) throw new IllegalArgumentException("Missing 'to' " +
            "keyword.");
